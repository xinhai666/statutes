...
<body>
		<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页<span
			class="c-gray en">&gt;</span> 修改法规 <a class="btn btn-success radius r"
			style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新"><i
				class="Hui-iconfont">&#xe68f;</i></a></nav>
<article class="page-container">
	<input type="button" class="btn" onclick="history.go(-1)" value="返回" /></span>
	<input type="hidden" id="statpId" value="${statute.statpId!}">
	<input type="hidden" id="stpId" value="${statute.staspId!}">
	<form class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
		<input type="hidden" id="statuteId" name="statuteId" value="${statute.statuteId!}">
		<div class="row c1">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" style="width: 30%" value="${statute.statutePartno!}" id="statutePartno" name="statutePartno">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">法规部号别名：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" style="width: 30%" value="${statute.statuteAlias!}" id="statuteAlias" name="statuteAlias">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" style="width: 30%" value="${statute.statuteVersion!}" id="statuteVersion" name="statuteVersion">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">*法规名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" style="width: 30%" value="${statute.statuteName!}" id="statuteName" name="statuteName">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">*是否显示 ：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<#if statute.statuteReveal==1>
				...
				<#else >
				<input type="radio" name="statuteReveal" value="1">显示
				<input type="radio" name="statuteReveal" value="0" checked style="margin-left:30px ">不显示
				</#if>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"  style="width:200px;">
				<#if statute.statuteIconpath==''>
					<img id="logoImg" src="image/file.png"; alt="暂无图片" style="height:120px;width:150px;" />
					<#else >
						<img id="logoImg" src="${statute.statuteIconpath!}"; alt="暂无图片" style="height:120px;width:150px;" />
				</#if>
				...
				<input type="hidden" value="${statute.statuteIconpath!}" id="statuteIconpath" name="statuteIconpath" style="height:120px;width:150px;">
			</div>
			<div class="text ">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text"  style="width: 30%"  value="${statute.statutePdfName!}" id="statutePdfName" name="statutePdfName">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">法规PDF文件下载链接：</label>
			<div class="col-xs-8 col-sm-9">
				<input id="statutePdfLink" readonly name="statutePdfLink"class="input-text" style="width: 30%" value="${statute.statutePdfLink!}">
		...
		<div class="row cl">
			...
			<textarea style="display:none" id="statuteExplain" name="statuteExplain">${statute.statuteExplain!}</textarea>
		</div>
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button class="btn btn-primary radius" type="button" id="subt"><i class="Hui-iconfont">&#xe632;</i> 保存修改</button>
			</div>
		</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		editor.create();
		editor.txt.html($("#statuteExplain").text());//设置文本编辑器内容
		...
		$.getJSON('getAllStatype',function (data) {
			for(var i=0;i<data.length;i++){
				var html="";
				if(data[i].statutestypeId==$("#statpId").val()){
					html+="<option selected value='"+data[i].statutestypeId+"'>"+data[i].statutestypeName+"</option>"
				}else {
					html+="<option value='"+data[i].statutestypeId+"'>"+data[i].statutestypeName+"</option>"
				}
				$("#statutestypeId").append(html);
			}
			getStatutesplit();
		});
		/*根据法规类型加载该类型所有主分支*/
	...
	function getStatutesplit() {
		$("#staspId option").slice(0).remove();
		...
		$.getJSON('getAllStaSplit',{'statutestypeId':statutestypeId},function (data) {
			if(data!=null){
				for(var i=0;i<data.length;i++){
					var html="";
					if(data[i].statutesplitId==stpId){
						html+="<option selected value='"+data[i].statutesplitId+"'>"+data[i].statutesplitName+"</option>"
					}else {
						html+="<option value='"+data[i].statutesplitId+"'>"+data[i].statutesplitName+"</option>"
					}
					$("#staspId").append(html);
				}
			}
